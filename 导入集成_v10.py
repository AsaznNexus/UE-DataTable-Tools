...
def merge_xlsx(input_path, output_path):
    ...
    # 新版策划表第三行保存字段类型；旧版策划表继续从第三行读取数据。
    has_type_row = (
        len(all_rows) >= 3 and
        str(all_rows[2][0]).strip() == "#FieldType"
    )
    row3 = list(all_rows[2]) if has_type_row else []
    data_start = 3 if has_type_row else 2
    ...
    if has_type_row:
        for ci, item in enumerate(plan):
            ...
            if marker is not None and str(marker).strip().upper() == "ARRAY":
                field_types[fname] = "ARRAY"
    ...
    for row in all_rows[data_start:]:
        ws_out.append(restore_row_v2(row, plan))
...
def parse_cell_value(val, force_array=False):
    """
...
def import_table(entry):
    ...
    try:
        ...
        # 新版合表文件在第二行保存字段类型；旧版文件继续按内容识别数组字段。
        has_type_row = str(ws.cell(row=2, column=1).value).strip() == "#FieldType"
        data_start = 3 if has_type_row else 2
        array_fields = set()

        if has_type_row:
            for col_idx, col_name in enumerate(headers, start=2):
                marker = ws.cell(row=2, column=col_idx).value
                if marker is not None and str(marker).strip().upper() == "ARRAY":
        ...
        for row in range(data_start, ws.max_row + 1):
            ...
            for col_idx, col_name in enumerate(headers, start=2):
                ...
                row_data[col_name] = parse_cell_value(
                    val, force_array=(col_name in array_fields)
                )
        ...
        if result:
            try:
                ...
                pass
            unreal.EditorAssetLibrary.save_asset(ue_asset_path)
...
if not run_preflight_check():
    ...
    if not table_list:
        ...
        unreal.log(f"\n合表完成：成功 {len(merge_success)} 个，失败 {len(merge_fail)} 个")
        ...
        if not merge_success:
            ...
            summary = (
                f"合表并导入完成！\n\n"
                f"【合表】成功 {len(merge_success)} 个，失败 {len(merge_fail)} 个\n"
