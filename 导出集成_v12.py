# -*- coding: utf-8 -*-
"""
导出并拆表 v2.5（导出集成_v12，UI 选表版，UE 5.7 兼容）
流程：
  ...
  7. 本地 SVN 路径存在时，合并复制 DataTables_Cehua
"""
...
# 导出完成后，将策划拆表结果合并复制到本地 SVN 工作目录。
# 仅在根路径已存在时执行，并保留目标中的 .svn 和其他已有文件。
SVN_CONFIG_ROOT = r"D:\JingYong_Design\03.配置表文档"
SVN_CEHUA_DEST = os.path.join(SVN_CONFIG_ROOT, "DataTables_Cehua")
...
# ══════════════════════════════════════════════════════
def sync_cehua_to_svn():
    """
    将 DataTables_Cehua 合并复制到本地 SVN 工作目录。

    - SVN 根路径不存在：直接跳过。
    - 目标 DataTables_Cehua 不存在：自动创建。
    - 目标已存在：覆盖同名文件，保留 .svn 和其他已有文件。

    返回 (status, message)，status 为 success / skipped / failed。
    """
    if not os.path.isdir(SVN_CONFIG_ROOT):
        message = f"已跳过（未找到 {SVN_CONFIG_ROOT}）"
        unreal.log(f"SVN 复制{message}")
        return "skipped", message

    if not os.path.isdir(CEHUA_BASE):
        message = f"失败（未找到源目录 {CEHUA_BASE}）"
        unreal.log_error(f"SVN 复制{message}")
        return "failed", message

    try:
        os.makedirs(SVN_CEHUA_DEST, exist_ok=True)
        shutil.copytree(CEHUA_BASE, SVN_CEHUA_DEST, dirs_exist_ok=True)
        copied_files = sum(
            len(file_names)
            for _, _, file_names in os.walk(CEHUA_BASE)
        )
        message = f"成功（{copied_files} 个文件 → {SVN_CEHUA_DEST}）"
        unreal.log(f"SVN 复制{message}")
...
try:
    ...
    if not all_table_entries:
        ...
        if selected_tables is None:
            ...
            svn_sync_status, svn_sync_msg = sync_cehua_to_svn()
            ...
            if svn_sync_status == "failed":
                all_fail.append("SVN 复制失败：\n" + svn_sync_msg)
            if all_fail:
                msg += "\n\n⚠️ 失败详情：\n" + "\n\n".join(all_fail)
